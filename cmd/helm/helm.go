...
// 处理完Chart路径寻找和下载任务后，就需要初始化Helm Client来准备与Tiller通信。
// ensureHelmClient函数就是为了完成这个功能而存在的，该函数接收一个client interface用来暂存Helm Client实例。
...
// 将Helm Client的各个参数传递过去，主要包含是否开启TlS访问，以及开启后证书的信息传递，最后配置一下超时时间等信息
// 这里其实仅完成了基本信息的配置，真正与Tllerr建立连接和访问还得等待后面真正有需求时才会工作。
